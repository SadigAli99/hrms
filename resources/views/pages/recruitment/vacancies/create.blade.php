...
@section('title', 'Vakansiya əlave et')

@section('content')
    <div class="page-block">
        @php
            $closedDateValue = old('closed_at', now()->format('Y-m-d'));
            $closedDateDisplay = $closedDateValue;
            $oldRequirements = old('vacancy_requirements', []);
        ...
        <form id="vacancyForm" class="space-y-6" method="post" action="{{ route('vacancy.store') }}">
            @csrf
            ...
            <div class="card">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input class="input" id="vacancyTitle" name="title" value="{{ old('title') }}">
                        @error('title')
                    ...
                    <div>
                        ...
                        <select class="input" id="vacancyEmploymentType" name="employment_type">
                            @foreach ($employment_types as $value => $employment_type)
                                <option value="{{ $value }}"
                                    {{ (string) old('employment_type') === $value ? 'selected' : '' }}>
                                    {{ $employment_type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select class="input" id="vacancyWorkMode" name="work_mode">
                            @foreach ($work_modes as $value => $work_mode)
                                <option value="{{ $value }}"
                                    {{ (string) old('work_mode') === $value ? 'selected' : '' }}>
                                    {{ $work_mode }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select class="input" id="vacancySeniority" name="seniority_level">
                            @foreach ($levels as $value => $level)
                                <option value="{{ $value }}"
                                    {{ (string) old('seniority_level') === $value ? 'selected' : '' }}>
                                    {{ $level }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select class="input" id="vacancyStatusInput" name="status">
                            @foreach ($statuses as $value => $status)
                                <option value="{{ $value }}"
                                    {{ (string) old('status') === $value ? 'selected' : '' }}>
                                    {{ $status }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input class="input" id="vacancyLocation" name="location" value="{{ old('location') }}">
                        @error('location')
                            <div class="mt-2 text-xs text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input class="input" id="vacancyMinSalary" name="min_salary" value="{{ old('min_salary') }}">
                        @error('min_salary')
                            <div class="mt-2 text-xs text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input class="input" id="vacancyMaxSalary" name="max_salary" value="{{ old('max_salary') }}">
                        @error('max_salary')
                            <div class="mt-2 text-xs text-red-400">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="card">
                    ...
                    <textarea class="input" id="vacancyDescription" name="description" rows="8">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="mt-2 text-xs text-red-400">{{ $message }}</div>
                    @enderror
                </div>
                <div class="card">
                    ...
                    <textarea class="input" id="vacancyRequirementsText" name="requirements_text" rows="8">{{ old('requirements_text') }}</textarea>
                    ...
                    <div class="requirement-list" id="requirementList">
                        @foreach ($oldRequirements as $index => $requirement)
                            @php
                                ...
                                $requirementRequired = filter_var($requirement['required'] ?? false, FILTER_VALIDATE_BOOLEAN);
                            @endphp
                            @if ($requirementLabel !== '')
                                <div class="requirement-pill {{ $requirementRequired ? 'is-required' : 'is-optional' }}"
                                    ...
                                    <div>
                                        ...
                                        <span>{{ $requirementType }} / {{ $requirementRequired ? 'required' : 'preferred' }}</span>
                                    </div>
