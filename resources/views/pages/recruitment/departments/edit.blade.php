...
@section('title', 'Departament yenilə')

@section('content')
    <div class="page-block">
        <form id="vacancyForm" class="space-y-6" method="post" action="{{ route('department.update', $department->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                        ...
                        <select class="input @error('manager_user_id') border-red-500/40 focus:border-red-500 @enderror"
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('manager_user_id', $department->manager_user_id) === $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select class="input @error('parent_id') border-red-500/40 focus:border-red-500 @enderror"
                            ...
                            @foreach ($departments as $dep)
                                <option value="{{ $dep->id }}"
                                    {{ old('parent_id', $department->parent_id) === $dep->id ? 'selected' : '' }}>
                                    {{ $dep->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select class="input @error('status') border-red-500/40 focus:border-red-500 @enderror"
                            ...
                            <option value="1"
                                {{ (string) old('status', $department->status) === '1' ? 'selected' : '' }}>Aktiv
                            </option>
                            <option value="0"
                                {{ (string) old('status', $department->status) === '0' ? 'selected' : '' }}>Deaktiv
                            </option>
                        </select>
                        @error('status')
                            <div class="mt-2 text-xs font-medium text-red-300">{{ $message }}</div>
