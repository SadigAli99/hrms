...
@section('title', 'İcazə yenilə')

@section('content')
    <div class="page-block">
        <form id="vacancyForm" class="space-y-6" method="post" action="{{ route('permission.update', $permission->id) }}">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <select class="input @error('status') border-red-500/40 focus:border-red-500 @enderror"
                            ...
                            <option value="1"
                                {{ (string) old('status', (string) $permission->status) === '1' ? 'selected' : '' }}>Aktiv
                            </option>
                            <option value="0"
                                {{ (string) old('status', (string) $permission->status) === '0' ? 'selected' : '' }}>Deaktiv
                            </option>
                        </select>
                        @error('status')
                            <div class="mt-2 text-xs font-medium text-red-300">{{ $message }}</div>
